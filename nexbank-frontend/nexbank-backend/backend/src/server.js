...
// CORS — set CLIENT_URL to your deployed frontend URL in production
...
// Initialize Express
const app = express();
// Trust reverse proxy (Render, Railway, Vercel, Replit, etc.)
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    callback(null, true); // permissive — lock via CLIENT_URL in production
  },
  credentials: true,
...
const startServer = async () => {
  try {
    if (!process.env.CLIENT_URL) {
      logger.warn(`CLIENT_URL not set — defaulting to ${clientUrl}`);
    }
