...
// Hash OTP with SHA-256 before storage — prevents plaintext exposure
const hashOTP = (otp) => crypto.createHash('sha256').update(String(otp)).digest('hex');

const storeOTP = async (key, otp, expiry = parseInt(process.env.OTP_EXPIRY) || 300) => {
  const redis = getRedis();
  const data = JSON.stringify({
    otp: hashOTP(otp),  // store hash, never plaintext
    attempts: 0,
    createdAt: Date.now()
  });
...
const verifyOTP = async (key, inputOTP) => {
  ...
  // Compare hash of submitted OTP against stored hash — never plaintext comparison
  if (parsed.otp !== hashOTP(inputOTP)) {
    const remaining = await redis.ttl(`otp:${key}`);
    if (remaining > 0) {
      await redis.setEx(`otp:${key}`, remaining, JSON.stringify(parsed));
    }
    return { valid: false, reason: 'Invalid OTP', attemptsLeft: maxAttempts - parsed.attempts };
  }

  // Invalidate immediately on success — prevent OTP reuse
  await redis.del(`otp:${key}`);
  return { valid: true };
};
